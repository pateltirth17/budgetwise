...
# Add these routes to your app.py temporarily

@app.route('/fix-database')
def fix_database():
    """Emergency route to create database tables"""
    import json
    
    result = {
        'step1_check_env': None,
        'step2_current_db': None,
        'step3_create_tables': None,
        'step4_verify': None
    }
    ...
    if db_url:
        result['step1_check_env'] = f"✓ DATABASE_URL is set: {db_url[:30]}..."
    else:
        result['step1_check_env'] = "✗ DATABASE_URL NOT SET!"
    
    # Step 2: Check current database
    result['step2_current_db'] = str(db.engine.url)[:50]
    
    # Step 3: Try to create tables
    try:
        db.create_all()
        result['step3_create_tables'] = "✓ db.create_all() executed"
    except Exception as e:
        result['step3_create_tables'] = f"✗ Error: {str(e)}"
    
    # Step 4: Verify tables exist
    try:
        # Try to query user table
        user_count = User.query.count()
        result['step4_verify'] = f"✓ User table exists with {user_count} users"
    except Exception as e:
        result['step4_verify'] = f"✗ User table still doesn't exist: {str(e)[:100]}"
    
    # Return results as JSON for clarity
    return f"""
    <html>
    ...
    <br>
    <h3>Next Steps:</h3>
    <p>1. If Step 4 shows '✓ User table exists', <a href="/signup">try signup now</a></p>
    <p>2. If still errors, <a href="/force-fix">click here for force fix</a></p>
    </body>
    </html>
    """
...
@app.route('/force-fix')
def force_fix():
    """Force create tables with detailed output"""
    output = []
    
    try:
        # Get database info
        db_url = os.environ.get('DATABASE_URL', 'Not set')
        output.append(f"DATABASE_URL from env: {db_url[:30] if db_url != 'Not set' else 'Not set'}...")
        output.append(f"Current DB URL: {str(db.engine.url)}")
        
        # Check if we're using PostgreSQL
        if 'postgresql' in str(db.engine.url):
            output.append("✓ Using PostgreSQL")
        else:
            output.append("⚠️ WARNING: Not using PostgreSQL!")
            ...
            if db_url and db_url != 'Not set':
                ...
                db.engine.dispose()  # Reset connection
                output.append("✓ Switched to PostgreSQL")
        
        # Drop and recreate all tables
        output.append("Dropping all tables...")
        db.drop_all()
        output.append("✓ Tables dropped")
        
        output.append("Creating all tables...")
        db.create_all()
        output.append("✓ Tables created")
        
        # Test with actual data
        output.append("Testing with sample user...")
        test_user = User()
        ...
        db.session.commit()
        output.append("✓ Test user created")
        ...
        output.append("You can now <a href='/signup'>sign up</a>")
    ...
    <h2>Force Fix Results:</h2>
    <pre style="background: white; padding: 15px; border-radius: 5px;">{'<br>'.join(output)}</pre>
    </body>
    </html>
    """
...
def check_db():
    ...
    try:
        ...
        info = {
            ...
            'Current DB': str(db.engine.url)[:50],
            'Is PostgreSQL': 'postgresql' in str(db.engine.url)
        }
        ...
        inspector = inspect(db.engine)
        tables = inspector.get_table_names()
        info['Tables'] = tables if tables else 'No tables found'
